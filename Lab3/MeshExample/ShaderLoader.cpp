...
bool ShaderLoad(GLhandleARB programId, char* shaderSrc, GLenum shaderType)
{
	...
	GLint str_len, max_len;
	GLint isCompiled = GL_FALSE;
	GLint isLinked = GL_FALSE;
	GLhandleARB h_shader;
	GLcharARB *pInfoLog;

	// First, create our shader
	...
	if ((fp = fopen(shaderSrc, "r")) == NULL){
		fprintf(stderr, "Fail to read the OpenGL shader source \%s\.\n", shaderSrc);
		return false;
	}

	// allocate memory for program string and load it.
	shader_string = (GLcharARB*)malloc(sizeof(GLcharARB)* 65536);
	str_len = (GLint)fread(shader_string, 1, 65536, fp);
	//printf("%d\n", str_len);
	//printf("%s\n", shader_string);

	glShaderSource(h_shader, 1, (const GLcharARB**)&shader_string, &str_len);
	free(shader_string);
	...
	if (!isCompiled){
		...
		glGetObjectParameterivARB(h_shader, GL_OBJECT_INFO_LOG_LENGTH_ARB, &max_len);
		pInfoLog = (GLcharARB *)malloc(max_len * sizeof(GLcharARB));
		glGetInfoLogARB(h_shader, max_len, &str_len, pInfoLog);
		fprintf(stderr, "%s\n", pInfoLog);
		free(pInfoLog);
		return false;
	}
	...
	if (!isLinked)
	{
		...
		glGetObjectParameterivARB(programId, GL_OBJECT_INFO_LOG_LENGTH_ARB, &max_len);
		pInfoLog = (GLcharARB *)malloc(max_len * sizeof(GLcharARB));
		glGetInfoLogARB(programId, max_len, &str_len, pInfoLog);
		fprintf(stderr, "%s\n", pInfoLog);
		free(pInfoLog);
		return false;
	}
